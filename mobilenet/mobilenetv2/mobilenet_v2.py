...
def inverted_res(network, weight_map, input, lname, inch, outch, s, exp):
    ...
    if exp != 1:
        ew1 = conv_bn_relu(
            network, weight_map, input, hidden, 1, 1, 1, f"{lname}conv.0."
        )
        ew2 = conv_bn_relu(
            network,
            weight_map,
        ...
        conv1 = network.add_convolution(
            ...
            num_output_maps=outch,
            kernel_shape=(1, 1),
            kernel=weight_map[f"{lname}conv.2.weight"],
            bias=trt.Weights(),
        )
        assert conv1
        bn1 = add_batch_norm_2d(
            network, weight_map, conv1.get_output(0), f"{lname}conv.3", EPS
        )
    else:
        ew1 = conv_bn_relu(
            network, weight_map, input, hidden, 3, s, hidden, f"{lname}conv.0."
        )
        conv1 = network.add_convolution(
            input=ew1.get_output(0),
            num_output_maps=outch,
            kernel_shape=(1, 1),
            kernel=weight_map[f"{lname}conv.1.weight"],
            bias=trt.Weights(),
        )
        assert conv1
        bn1 = add_batch_norm_2d(
            network, weight_map, conv1.get_output(0), f"{lname}conv.2", EPS
        )
